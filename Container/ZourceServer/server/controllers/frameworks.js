...
async function create(ctx) {
    ...
    if (component) {
        ctx.body = util.format('Already exists: %s (%s)', name, version)
        return
    }
    ...
    ctx.body = util.format('Save succefully: %s (%s)', name, version)
}
...
async function destroy(ctx) {
    ...
    if (!component) {
        ctx.status = 404
        ctx.body = util.format('no binary file: %s (%s)', name, version)
        return
    }
    ...
    ctx.body = util.format('Delete succefully: %s (%s)', name, version)
}
